...
async function getSettingArrays(v: ThemeConfig) {
    ...
    // 导出时显示块样式
    settings.push({ label: localMessage["blockstyleitem"][defLag], description: localMessage["blockstyledesc"][defLag], id: 'exportBlockStyle', enable: v?.plugins?.exportBlockStyle ?? false });
    return settings;
}
