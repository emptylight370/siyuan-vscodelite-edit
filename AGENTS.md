...
- `pnpm run build:ts`：仅编译TypeScript文件，使用Rollup打包到`theme.js`
- `pnpm run watch`：启动开发监视模式，自动重新编译SCSS和TypeScript
- `pnpm run watch:scss`：仅监视SCSS文件变化
- `pnpm run watch:ts`：仅监视TypeScript文件变化
...
3. **构建系统**
   - `rollup.config.js`：TypeScript打包配置，输出为IIFE格式
   - SCSS通过sass编译器直接输出，支持source map
   - 使用npm-run-all并行执行构建任务
