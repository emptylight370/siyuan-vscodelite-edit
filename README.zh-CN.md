...
|     `row-bg`      | 表格块       | 为偶数行添加背景色                     | 3.0.12   | 3.0.12   |
|     `col-bg`      | 表格块       | 为偶数列添加背景色                     | 3.0.12   | 3.0.12   |
...
- 快捷键面板（`siyuan-plugin-keymap`）（分类标题颜色）
- 自定义块样式（`custom-block`）（修复由主题造成的嵌入块中自定义块显示问题）
- 替换背景图（`siyuan-plugin-background-cover`）（通过覆写部分插件设置实现，相关设置不生效属正常现象）
- 数学增强（`siyuan-plugin-math-enhance`）（限制插件预览宽度）
